...
public class Aparecer : MonoBehaviour
{
    ...
    public bool OrientarHaciaEnemigo = true;
    ...
    private void AsignarVelocidadMovimiento()
    {
        ...
        //--- Apuntar hacia al jugador--------------------------------------
        if (OrientarHaciaEnemigo)
        {
            var dir = jugadorObjetivo.transform.position - transform.position;
            float angle = (Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg) + 180f; //180 extra pues punta del sprite en lado izquierdo        
            transform.rotation = new Quaternion();
        ...
        else
        {
            if (OrientarHaciaEnemigo)
                ...
                transform.Rotate(new Vector3(0, 1, 0), 0f);
