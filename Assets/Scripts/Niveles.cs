...
public class Niveles : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < listaNiveles.Count; i++)
        {
            ...
            {
                listaNiveles[i].imagenNivel = nivelDesbloquedo;
                //listaNiveles[i].objetoNivel.gameObject.SetActive(true);

                if (listaNiveles[i].estrellas == 1) listaNiveles[i].imagenEstrellas = unaEstrella;
                if (listaNiveles[i].estrellas == 2) listaNiveles[i].imagenEstrellas = dosEstrellas;
                if (listaNiveles[i].estrellas == 3) listaNiveles[i].imagenEstrellas = tresEstrellas;
            }
            else
            {
